...
from models.transaction_group_model import TransactionGroup, TransactionGroupDB
...
async def create_type(group: TransactionGroup, db: AsyncSession = Depends(get_db)):
    # check user exists
...
async def get_knowledgebase_category(group_id: int, db: AsyncSession = Depends(get_db)):
...
async def update_category(group_id: int, group_update: TransactionGroup, db: AsyncSession = Depends(get_db)):
    result = await db.execute(
        select(TransactionGroupDB)
