...
async def create_stage(status: ReviewStage, db: AsyncSession = Depends(get_db)):
    ...
    try:
        ...
        await db.rollback()
        raise HTTPException(status_code=400, detail=f"Unable to create review stage: f{e}")
    return db_user

@router.post("/initialize")
async def initialize(db: AsyncSession = Depends(get_db)):
    stageList = ['Awaiting Submission', 'Primary Approval', 'Secondary Approval', 'Approved']
    stageId = 1
    ...
    try:
        ...
        await db.rollback()
        raise HTTPException(status_code=400, detail=f"Unable to initialize review stages: f{e}")
    return {'succeeded': True, 'message': 'Review stages have been successfully initialized'}
