...
from models.payment_method_model import PaymentMethodDB, PaymentMethod, PaymentMethodWithDetail
...
async def post_paymentmethod(paymentmethod: PaymentMethod, db: AsyncSession = Depends(get_db)):
    # check user exists
    result = await db.execute(select(MemberDB).where(MemberDB.id == paymentmethod.user_id))
    member = result.scalars().first()
    
    if not member:
        raise HTTPException(
            status_code=400, detail=f"The member with id '{paymentmethod.user_id}' does not exist"
        )
    ...
    try:
        ...
        await db.rollback()
        raise HTTPException(status_code=400, detail=f"Unable to create payment method: {e}")
    return db_tran
...
async def update_item(config_id: int, config_update: PaymentMethod, db: AsyncSession = Depends(get_db)):
    ...
    if not config:
        raise HTTPException(status_code=404, detail=f"Unable to find paymentmethod with id '{config_id}'")
    ...
    try:
        ...
        await db.rollback()
        raise HTTPException(status_code=400, detail=f"Unable to update paymentmethod {e}")
    return config


@router.get("/list", response_model=List[PaymentMethodWithDetail])
async def list_paymentmethods(db: AsyncSession = Depends(get_db)):
    result = await db.execute(
        select(PaymentMethodDB)
    )
...
async def list_my_paymentmethods(user_id: int, db: AsyncSession = Depends(get_db)):
    result = await db.execute(
        select(PaymentMethodDB)
...
async def get_paymentmethod(paymentmethod_id: int, db: AsyncSession = Depends(get_db)):
    result = await db.execute(
        select(PaymentMethodDB)
        .filter(PaymentMethodDB.id == paymentmethod_id)
    )
    paymentmethod = result.scalars().first()
    if not paymentmethod:
        raise HTTPException(status_code=404, detail=f"Unable to find paymentmethod with id '{paymentmethod_id}'")
...
async def review_posting(
    ...
    if paymentmethod.stage_id == assist.APPROVAL_STAGE_SUBMITTED:
        ...
        if review.review_action == assist.REVIEW_ACTION_REJECT:
            ...
            elif paymentmethod.approval_levels == 2 or paymentmethod.approval_levels == 3:
