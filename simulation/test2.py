...
#SELF CASE FOR BALANCE
random.randint(1, 3)
#lit of all members
print('1-----------------------------------------------------------------')
all_members = ['Mike', 'Ruth', 'Sara']

#contributions = {m: random.randint(*(10, 70)) for m in all_members}
contributions = {'Mike': 10, 'Ruth': 31, 'Sara': 59}

print('contribitions', contributions)
...
#loan_requests = {m: random.randint(*(30, 50)) for m in applicants}
loan_requests = {'Ruth': 43, 'Sara': 43}

print('Total members', len(all_members), ' Applicants', len(loan_requests))
...
#print('member_totals=', member_totals)

for key, value in member_totals.items():
    print('-----------------------------------------------------------------------------------------------------------------------')
    print('name:', key, ', contribution=', contributions[key], ', loans received=', value['received'], ', loans requested=', value['loan'],', loans balance=', value['loan-balance'],', saving balance=', value['saving-balance'])

##member_summary = {key: {"contributed": value['contributed'], "received": value['received'], "balance": contributions[key]-value['contributed']} for key, value in member_totals.items()}
##print('member_summary=', member_summary)
