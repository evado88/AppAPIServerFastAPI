...
from apps.ccl.models.login_model import LoginDB
from apps.ccl.models.user_model import User, UserDB
...
@router.post("/login")
async def login(
    form_data: OAuth2PasswordRequestForm = Depends(), db: AsyncSession = Depends(get_ccl_db)
):
    # check user exists
