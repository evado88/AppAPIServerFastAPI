...
from apps.osawe.models.meeting_model import Meeting, MeetingDB, MeetingSimpleWithDetail, MeetingWithDetail
from apps.osawe.models.review_model import SACCOReview
from apps.osawe.models.transaction_model import TransactionDB
...
async def post_meeting(meeting: Meeting, db: AsyncSession = Depends(get_osawe_db)):
    # check user exists
...
async def list_meetings(db: AsyncSession = Depends(get_osawe_db)):
...
async def list_status_meetings(statusId: int, db: AsyncSession = Depends(get_osawe_db)):
    result = await db.execute(
        select(
            MeetingDB
        ) .filter(
...
async def update_meeting(
    id: int, meeting_update: Meeting, db: AsyncSession = Depends(get_osawe_db)
):
...
async def get_meeting(meeting_id: int, db: AsyncSession = Depends(get_osawe_db)):
    result = await db.execute(
        select(MeetingDB)
        # .options(
