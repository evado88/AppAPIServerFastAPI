...
from apps.osawe.models.notification_model import Notification, NotificationDB, NotificationWithDetail
...
async def post_notification(notificcation: Notification, db: AsyncSession = Depends(get_osawe_db)):
    # check user exists
...
async def list_notifications(db: AsyncSession = Depends(get_osawe_db)):
...
async def get_notification(notification_id: int, db: AsyncSession = Depends(get_osawe_db)):
    result = await db.execute(
        select(NotificationDB)
        #.options(
        #    joinedload(NotificationDB.post),
