...
from apps.osawe.models.configuration_model import (
    SACCOConfiguration,
    SACCOConfigurationDB,
    SACCOConfigurationWithDetail,
)
...
async def post_config(config: SACCOConfiguration, db: AsyncSession = Depends(get_osawe_db)):
    # check user exists
    ...
    try:
        ...
        await db.rollback()
        raise HTTPException(
            status_code=400, detail=f"Unable to create configuration {e}"
        )
    return db_tran
...
async def update_item(
    ...
    if not config:
        raise HTTPException(
            status_code=404,
            detail=f"Unable to find configuration with id '{config_id}' not found",
        )
    ...
    try:
        ...
        await db.rollback()
        raise HTTPException(
            status_code=400, detail=f"Unable to update configuration {e}"
        )
    return config


@router.get("/", response_model=List[SACCOConfigurationWithDetail])
async def list_configurations(db: AsyncSession = Depends(get_osawe_db)):
    result = await db.execute(
...
async def get_configuration(config_id: int, db: AsyncSession = Depends(get_osawe_db)):
    result = await db.execute(
