...
from apps.osawe.models.attendance_types_model import AttendanceType, AttendanceTypeDB

router = APIRouter(prefix="/attendance-types", tags=["AttendanceTypes"])


@router.post("/create", response_model=AttendanceType)
async def create_type(status: AttendanceType, db: AsyncSession = Depends(get_osawe_db)):
