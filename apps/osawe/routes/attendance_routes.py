...
from apps.osawe.models.attendance_model import Attendance, AttendanceDB
...
async def post_attendance(attendance: Attendance, db: AsyncSession = Depends(get_osawe_db)):
    # check user exists
...
async def list_attendances(db: AsyncSession = Depends(get_osawe_db)):
...
async def get_attendance(attendance_id: int, db: AsyncSession = Depends(get_osawe_db)):
    result = await db.execute(
        select(AttendanceDB)
        #.options(
        #    joinedload(AttendanceDB.post),
