...
from apps.osawe.models.knowledge_base_category_model import KnowledgeBaseCategory, KnowledgeBaseCategoryDB
...
async def post_knowledgebase_category(category: KnowledgeBaseCategory, db: AsyncSession = Depends(get_osawe_db)):
    # check user exists
...
async def update_category(cat_id: int, category_update: KnowledgeBaseCategory, db: AsyncSession = Depends(get_osawe_db)):
...
async def list_knowledgebase_categories(db: AsyncSession = Depends(get_osawe_db)):
    result = await db.execute(
        select(KnowledgeBaseCategoryDB)
        #.options(
...
async def get_knowledgebase_category(cat_id: int, db: AsyncSession = Depends(get_osawe_db)):
    result = await db.execute(
        select(KnowledgeBaseCategoryDB)
        #.options(
        #    joinedload(KnowledgeBaseCategoryDB.post),
