...
from apps.osawe.models.monthly_post_model import MonthlyPostingDB
...
from apps.osawe.models.coomunication_channel_models import AuthParam, MobileParam, ItemReviewParam, TransactionReviewParam
...
async def send_account_registered(
    param: ItemReviewParam, db: AsyncSession = Depends(get_osawe_db)
):
    # check user exists
...
async def send_account_approved(param: ItemReviewParam, db: AsyncSession = Depends(get_osawe_db)):
    # check user exists
    result = await db.execute(select(MemberDB).where(MemberDB.id == param.id))
    member = result.scalars().first()
