...
from apps.osawe.models.configuration_model import SACCOConfigurationDB
from apps.osawe.models.guarantor_model import GuarantorDB
from apps.osawe.models.member_model import MemberDB
from apps.osawe.models.monthly_post_model import (
    MonthlyPosting,
    MonthlyPostingDB,
    MonthlyPostingWithDetail,
    MonthlyPostingWithMemberDetail,
)
from apps.osawe.models.param_models import ParamMonthlyPosting
from apps.osawe.models.payment_method_model import PaymentMethodDB
from apps.osawe.models.posting_period_model import PostingPeriodDB
from apps.osawe.models.review_model import SACCOReview
from apps.osawe.models.transaction_model import Transaction, TransactionDB
...
async def post_posting(posting: MonthlyPosting, db: AsyncSession = Depends(get_osawe_db)):
    # check user exists
...
async def review_posting(
    post_id: int, review: SACCOReview, db: AsyncSession = Depends(get_osawe_db)
):
    # check user exists
...
async def list_postings(db: AsyncSession = Depends(get_osawe_db)):
    result = await db.execute(
...
async def list_user_postings(userId: int, db: AsyncSession = Depends(get_osawe_db)):
    result = await db.execute(
        select(MonthlyPostingDB)
        # .options(
        #    joinedload(MonthlyPostingDB.status),
...
async def list_user_mid_postings(userId: int, db: AsyncSession = Depends(get_osawe_db)):
    result = await db.execute(
        select(MonthlyPostingDB)
        # .options(
        #    joinedload(MonthlyPostingDB.status),
...
async def list_user_postings(email: str, db: AsyncSession = Depends(get_osawe_db)):
    result = await db.execute(
        select(MonthlyPostingDB)
        # .options(
        #    joinedload(MonthlyPostingDB.status),
...
async def list_status_postings(status_id: int, db: AsyncSession = Depends(get_osawe_db)):
    result = await db.execute(
        select(MonthlyPostingDB)
        # .options(
        #    joinedload(MonthlyPostingDB.status),
...
async def list_period_postings(period_id: str, db: AsyncSession = Depends(get_osawe_db)):
    result = await db.execute(
        select(MonthlyPostingDB)
        # .options(
        #    joinedload(MonthlyPostingDB.status),
...
async def list_status_period_postings(
    period_id: str, status_id: int, db: AsyncSession = Depends(get_osawe_db)
):
    result = await db.execute(
        select(MonthlyPostingDB)
        # .options(
...
async def get_posting(posting_id: int, db: AsyncSession = Depends(get_osawe_db)):
    result = await db.execute(
        select(MonthlyPostingDB)
        # .options(
        #    joinedload(MonthlyPostingDB.status),
...
async def update_posting(
    id: int, posting_update: MonthlyPosting, db: AsyncSession = Depends(get_osawe_db)
):
