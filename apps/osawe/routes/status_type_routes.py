...
from apps.osawe.models.status_types_model import StatusType, StatusTypeDB

router = APIRouter(prefix="/status-types", tags=["Statuses"])


@router.post("/create", response_model=StatusType)
async def create_status(status: StatusType, db: AsyncSession = Depends(get_osawe_db)):
...
async def list_statuses(db: AsyncSession = Depends(get_osawe_db)):
