...
from apps.osawe.models.transaction_states_model import TransactionState, TransactionStateDB

router = APIRouter(prefix="/transaction-states", tags=["TransactionStates"])


@router.post("/create", response_model=TransactionState)
async def create_states(transaction: TransactionState, db: AsyncSession = Depends(get_osawe_db)):
