...
from apps.osawe.models.transaction_sources_model import TransactionSource, TransactionSourceDB

router = APIRouter(prefix="/transaction-sources", tags=["TransactionSources"])


@router.post("/create", response_model=TransactionSource)
async def create_status(transaction: TransactionSource, db: AsyncSession = Depends(get_osawe_db)):
...
async def list_sources(db: AsyncSession = Depends(get_osawe_db)):
