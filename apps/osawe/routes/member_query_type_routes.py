...
from apps.osawe.models.member_query_type_model import MemberQueryType, MemberQueryTypeDB

router = APIRouter(prefix="/member-query-types", tags=["MemberQueryTypes"])


@router.post("/create", response_model=MemberQueryType)
async def create_type(status: MemberQueryType, db: AsyncSession = Depends(get_osawe_db)):
