...
from apps.osawe.models.annoucement_model import Announcement, AnnouncementDB, AnnouncementWithDetail
...
async def post_announcement(announcement: Announcement, db: AsyncSession = Depends(get_osawe_db)):
    # check user exists
...
async def update_item(config_id: int, config_update: Announcement, db: AsyncSession = Depends(get_osawe_db)):
...
async def list_announcements(db: AsyncSession = Depends(get_osawe_db)):
    result = await db.execute(
        select(AnnouncementDB)
        #.options(
...
async def list_recent_announcements(db: AsyncSession = Depends(get_osawe_db)):
    result = await db.execute(
        select(AnnouncementDB)
        .where(
...
async def get_announcement(announcement_id: int, db: AsyncSession = Depends(get_osawe_db)):
    result = await db.execute(
        select(AnnouncementDB)
        #.options(
        #    joinedload(AnnouncementDB.post),
