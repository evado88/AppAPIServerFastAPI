...
from apps.osawe.models.monthly_post_model import MonthlyPostingDB, MonthlyPostingWithMemberDetail
from apps.osawe.models.param_models import ParamPeriodSummary
from apps.osawe.models.posting_period_model import (
    PostingPeriod,
    PostingPeriodDB,
    PostingPeriodWithDetail,
)
from apps.osawe.models.review_model import SACCOReview
from apps.osawe.models.review_stages_model import ReviewStageDB
...
async def create_period(period: PostingPeriod, db: AsyncSession = Depends(get_osawe_db)):
...
async def list_periods(db: AsyncSession = Depends(get_osawe_db)):
...
async def update_meeting(
    id: str, period_update: PostingPeriod, db: AsyncSession = Depends(get_osawe_db)
):
...
async def get_posting(period_id: str, db: AsyncSession = Depends(get_osawe_db)):
...
async def review_posting(
    post_id: str, review: SACCOReview, db: AsyncSession = Depends(get_osawe_db)
):
    # check user exists
...
async def list_current_periods(
    year: int, month: int, status_id: int, db: AsyncSession = Depends(get_osawe_db)
):
...
async def list_current_periods(period_id: str, db: AsyncSession = Depends(get_osawe_db)):
    result = await db.execute(
