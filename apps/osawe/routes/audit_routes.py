...
from apps.osawe.models.audit_model import Audit, AuditDB, AuditSimpleWithDetail, AuditWithDetail
...
async def post_audit(audit: Audit, db: AsyncSession = Depends(get_osawe_db)):
    # check user exists
...
async def list_audits(db: AsyncSession = Depends(get_osawe_db)):
