...
from apps.osawe.models.knowledge_base_model import (
    KnowledgeBase,
    KnowledgeBaseDB,
...
async def post_kbarticle(kbarticle: KnowledgeBase, db: AsyncSession = Depends(get_osawe_db)):
    # check user exists
...
async def update_item(
    config_id: int, config_update: KnowledgeBase, db: AsyncSession = Depends(get_osawe_db)
):
...
async def list_kbarticles(db: AsyncSession = Depends(get_osawe_db)):
    result = await db.execute(
...
async def list_recent_kbarticles(db: AsyncSession = Depends(get_osawe_db)):
    result = await db.execute(
        select(KnowledgeBaseDB)
        .where(
...
async def get_kbarticle_id(kbarticle_id: int, db: AsyncSession = Depends(get_osawe_db)):
    result = await db.execute(
        select(KnowledgeBaseDB)
        # .options(
        #    joinedload(KnowledgeBaseDB.post),
