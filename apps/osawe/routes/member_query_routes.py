...
from apps.osawe.models.member_query_model import MemberQuery, MemberQueryDB, MemberQueryWithDetail
...
async def post_memberquery(mquery: MemberQuery, db: AsyncSession = Depends(get_osawe_db)):
    # check user exists
...
async def list_memberquerys(db: AsyncSession = Depends(get_osawe_db)):
...
async def get_memberquery(memberquery_id: int, db: AsyncSession = Depends(get_osawe_db)):
    result = await db.execute(
        select(MemberQueryDB)
        # .options(
        #    joinedload(MemberQueryDB.post),
...
async def update_item(
    query_id: int, config_update: MemberQuery, db: AsyncSession = Depends(get_osawe_db)
):
...
async def list_user_memberquery(user_id: int, db: AsyncSession = Depends(get_osawe_db)):
    result = await db.execute(
        select(MemberQueryDB)
        # .options(
        #    joinedload(MemberQueryDB.post),
