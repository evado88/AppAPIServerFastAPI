...
from apps.osawe.models.param_models import (
...
from apps.osawe.models.posting_period_model import PostingPeriodDB
from apps.osawe.models.review_model import SACCOReview
from apps.osawe.models.transaction_model import Transaction, TransactionDB, TransactionWithDetail
from apps.osawe.models.user_model import UserDB
from apps.osawe.models.transaction_types_model import TransactionTypeDB
from apps.osawe.models.transaction_sources_model import TransactionSourceDB
from apps.osawe.models.status_types_model import StatusTypeDB
import pandas as pd
import numpy as np
...
async def post_transaction(tran: Transaction, db: AsyncSession = Depends(get_osawe_db)):
    # check user exists
...
async def list_transactions(db: AsyncSession = Depends(get_osawe_db)):
...
async def list_user_status_transactions(
    userId: int, typeId: int, statusId: int, db: AsyncSession = Depends(get_osawe_db)
):
    result = await db.execute(
        select(TransactionDB)
        .filter(
...
async def list_status_transactions(typeId: int, statusId: int, year: int, db: AsyncSession = Depends(get_osawe_db)
):
...
async def list_expense_earnings_status_transactions(
    statusId: int, db: AsyncSession = Depends(get_osawe_db)
):
    result = await db.execute(
        select(TransactionDB)
        .filter(
...
async def update_transaction(
    id: int, transaction_update: Transaction, db: AsyncSession = Depends(get_osawe_db)
):
...
async def get_member_summary(
    userId: int, year: int, db: AsyncSession = Depends(get_osawe_db)
):
    # year
    start_of_year = date(year, 1, 1)
    start_of_next_year = date(year + 1, 1, 1)
...
async def get_all_member_summary(year: int, db: AsyncSession = Depends(get_osawe_db)):
    # get all transaction types
    result = await db.execute(select(TransactionTypeDB))
    types = result.scalars().all()
...
async def get_all_member_interest_sharing(year: int, db: AsyncSession = Depends(get_osawe_db)):
...
async def get_all_member_payout_sharing(year: int, db: AsyncSession = Depends(get_osawe_db)):
    print("starting all member interest sharing", assist.get_current_date(False))
...
async def get_all_member_transaction_summary(year: int, db: AsyncSession = Depends(get_osawe_db)):
...
async def get_all_expense_earnings_summary(year: int, db: AsyncSession = Depends(get_osawe_db)):
...
async def initialize_validation_savings(db: AsyncSession = Depends(get_osawe_db)):
...
async def initialize_validation_loans(db: AsyncSession = Depends(get_osawe_db)):
...
async def initialize_validation_interest(db: AsyncSession = Depends(get_osawe_db)):
...
async def initialize_validation_loan_repayment(db: AsyncSession = Depends(get_osawe_db)):
