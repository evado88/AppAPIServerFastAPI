...
from apps.tpsuperapp.models.customer_model import (
    Customer,
    CustomerDB,
    CustomerItem,
    CustomerSimple,
...
from apps.tpsuperapp.models.param_models import ParamCustomer, ParamCustomerImport
...
async def create_type(customer: Customer, db: AsyncSession = Depends(get_tpsuperapp_db)):
    # check user exists
...
async def get_customer(customer_id: int, db: AsyncSession = Depends(get_tpsuperapp_db)):
    result = await db.execute(
        select(CustomerDB)
        .options(
...
async def get_customer_phone(customer_phone: str, db: AsyncSession = Depends(get_tpsuperapp_db)):
    result = await db.execute(
        select(CustomerDB)
        .options(
            selectinload(CustomerDB.user),
...
async def get_edit_customer(customer_id: int, db: AsyncSession = Depends(get_tpsuperapp_db)):
    # get customer
    result = await db.execute(
...
async def update_category(
    customer_id: int, customer_update: Customer, db: AsyncSession = Depends(get_tpsuperapp_db)
):
    result = await db.execute(
        select(CustomerDB)
        .options(
...
async def list_customers(db: AsyncSession = Depends(get_tpsuperapp_db)):
    result = await db.execute(
        select(CustomerDB)
        .options(
            selectinload(CustomerDB.user),
...
async def list_customers(routeId: int, db: AsyncSession = Depends(get_tpsuperapp_db)):
    result = await db.execute(
        select(CustomerDB)
        .options(
            selectinload(CustomerDB.category),
