...
from apps.tpsuperapp.models.review_stages_model import ReviewStage, ReviewStageDB

router = APIRouter(prefix="/review-stages", tags=["ReviewStages"])


@router.post("/create", response_model=ReviewStage)
async def create_stage(status: ReviewStage, db: AsyncSession = Depends(get_tpsuperapp_db)):
