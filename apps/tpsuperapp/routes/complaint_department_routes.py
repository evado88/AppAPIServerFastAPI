...
from apps.tpsuperapp.models.complaint_department_model import (
    ComplaintDepartment,
...
async def post__category(
    category: ComplaintDepartment, db: AsyncSession = Depends(get_tpsuperapp_db)
):
    # check user exists
...
async def list_categories(db: AsyncSession = Depends(get_tpsuperapp_db)):
    result = await db.execute(
        select(ComplaintDepartmentDB)
        .options(
            selectinload(ComplaintDepartmentDB.user),
...
async def list_items(db: AsyncSession = Depends(get_tpsuperapp_db)):
    result = await db.execute(
        select(ComplaintDepartmentDB)
        .options(
            selectinload(ComplaintDepartmentDB.user),
...
async def get_category(cat_id: int, db: AsyncSession = Depends(get_tpsuperapp_db)):
    result = await db.execute(
        select(ComplaintDepartmentDB)
        .options(
            selectinload(ComplaintDepartmentDB.user),
