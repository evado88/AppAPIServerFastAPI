...
from apps.tpsuperapp.models.meter_status_model import (
    MeterStatus,
    MeterStatusDB,
    MeterStatusSimple,
...
async def post_meter_status(
    meterstatus: MeterStatus, db: AsyncSession = Depends(get_tpsuperapp_db)
):
    # check user exists
...
async def list_meter_statuses(db: AsyncSession = Depends(get_tpsuperapp_db)):
    result = await db.execute(
...
async def list_meter_statuses_simple(db: AsyncSession = Depends(get_tpsuperapp_db)):
    result = await db.execute(
        select(MeterStatusDB).options(
            selectinload(MeterStatusDB.user),
        )
...
async def get_meter_status(status_id: int, db: AsyncSession = Depends(get_tpsuperapp_db)):
    result = await db.execute(
        select(MeterStatusDB)
        .options(
            selectinload(MeterStatusDB.user),
