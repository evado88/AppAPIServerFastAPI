...
from apps.tpsuperapp.models.district_model import (
    District,
    DistrictDB,
    DistrictSimple,
    DistrictWithDetail,
...
async def post_district(district: District, db: AsyncSession = Depends(get_tpsuperapp_db)):
    # check user exists
...
async def update_district(
    district_id: int, district_update: District, db: AsyncSession = Depends(get_tpsuperapp_db)
):
...
async def list__districts(db: AsyncSession = Depends(get_tpsuperapp_db)):
    result = await db.execute(
        select(DistrictDB)
        .options(
            selectinload(DistrictDB.stage),
...
async def get__district(district_id: int, db: AsyncSession = Depends(get_tpsuperapp_db)):
    result = await db.execute(
        select(DistrictDB)
        .options(
            selectinload(DistrictDB.stage),
