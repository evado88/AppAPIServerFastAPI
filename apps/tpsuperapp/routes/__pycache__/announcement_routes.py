...
from apps.tpsuperapp.models.annoucement_model import Announcement, AnnouncementDB, AnnouncementWithDetail
from apps.tpsuperapp.models.attachment_model import AttachmentDB
from apps.tpsuperapp.models.review_model import SACCOReview
...
async def post_announcement(announcement: Announcement, db: AsyncSession = Depends(get_tpsuperapp_db)):
    # check user exists
...
async def update_configuration(config_id: int, config_update: Announcement, db: AsyncSession = Depends(get_tpsuperapp_db)):
...
async def list_announcements(db: AsyncSession = Depends(get_tpsuperapp_db)):
    result = await db.execute(
        select(AnnouncementDB)
        #.options(
...
async def list_recent_announcements(db: AsyncSession = Depends(get_tpsuperapp_db)):
    result = await db.execute(
        select(AnnouncementDB)
        .where(
...
async def get_announcement(announcement_id: int, db: AsyncSession = Depends(get_tpsuperapp_db)):
    result = await db.execute(
        select(AnnouncementDB)
        #.options(
        #    joinedload(AnnouncementDB.post),
...
async def review_posting(
    id: int, review: SACCOReview, db: AsyncSession = Depends(get_tpsuperapp_db)
):
    # check user exists
