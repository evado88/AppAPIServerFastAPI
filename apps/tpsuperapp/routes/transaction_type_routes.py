...
from apps.tpsuperapp.models.transaction_type_model import TransactionType, TransactionTypeDB

router = APIRouter(prefix="/transaction-types", tags=["TransactionTypes"])


@router.post("/create", response_model=TransactionType)
async def create_type(transaction: TransactionType, db: AsyncSession = Depends(get_tpsuperapp_db)):
...
async def list_types(db: AsyncSession = Depends(get_tpsuperapp_db)):
