...
from apps.tpsuperapp.models.param_models import (
    ParamChartItem,
...
async def get_ytd_dashboard(year: int, db: AsyncSession = Depends(get_tpsuperapp_db)):
    ...
    stmt = (
        ...
        .where(
            MeterReadingDB.status_id == tpsuperapp.APPROVAL_STAGE_APPROVED,
    ...
    stmt = select(func.count(UserDB.id)).where(
        UserDB.status_id == tpsuperapp.APPROVAL_STAGE_APPROVED
    )
    ...
    stmt = select(func.count(DistrictDB.id)).where(
        DistrictDB.status_id == tpsuperapp.APPROVAL_STAGE_APPROVED
    )
    ...
    stmt = select(func.count(WalkRouteDB.id)).where(
        WalkRouteDB.status_id == tpsuperapp.APPROVAL_STAGE_APPROVED
    )
    ...
    stmt = select(func.count(CustomerDB.id)).where(
        CustomerDB.status_id == tpsuperapp.APPROVAL_STAGE_APPROVED
    )
