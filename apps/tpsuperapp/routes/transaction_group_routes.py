...
from apps.tpsuperapp.models.transaction_group_model import (
...
async def create_group(
    group: TransactionGroup, db: AsyncSession = Depends(get_tpsuperapp_db)
):
    # check user exists
...
async def get_group(group_id: int, db: AsyncSession = Depends(get_tpsuperapp_db)):
...
async def list_groups(db: AsyncSession = Depends(get_tpsuperapp_db)):
    result = await db.execute(
        select(TransactionGroupDB)
        .options(
            selectinload(TransactionGroupDB.type),
