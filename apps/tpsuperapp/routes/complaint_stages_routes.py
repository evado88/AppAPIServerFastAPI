...
from apps.tpsuperapp.models.complaint_stages_model import ComplaintStage, ComplaintStageDB

router = APIRouter(prefix="/complaint-stages", tags=["ComplaintStages"])


@router.post("/create", response_model=ComplaintStage)
async def create_stage(status: ComplaintStage, db: AsyncSession = Depends(get_tpsuperapp_db)):
...
async def list_stages(db: AsyncSession = Depends(get_tpsuperapp_db)):
