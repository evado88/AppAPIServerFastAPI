...
from apps.tpsuperapp.models.transaction_group_model import TransactionGroupDB
from apps.tpsuperapp.models.transaction_model import (
    MobileTransaction,
...
from apps.tpsuperapp.models.transaction_type_model import TransactionTypeDB
...
async def post_transaction(tran: Transaction, db: AsyncSession = Depends(get_tpsuperapp_db)):
    # check user exists
...
async def list_transactions(db: AsyncSession = Depends(get_tpsuperapp_db)):
    result = await db.execute(
...
async def get_transaction(tran_id: int, db: AsyncSession = Depends(get_tpsuperapp_db)):
...
async def update_transaction(
    id: int, transaction_update: Transaction, db: AsyncSession = Depends(get_tpsuperapp_db)
):
    result = await db.execute(
        select(TransactionDB)
        .options(
