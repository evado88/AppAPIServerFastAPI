...
from apps.tpsuperapp.models.param_models import ParamCustomerImport, ParamUserEdit
from apps.tpsuperapp.models.walkroute_model import (
    WalkRouteDB,
    WalkRouteItem,
...
from apps.tpsuperapp.models.user_model import User, UserDB, UserSimple, UserWithDetail

router = APIRouter(prefix="/users", tags=["Users"])


@router.post("/create", response_model=User)
async def create_user(user: User, db: AsyncSession = Depends(get_tpsuperapp_db)):
    ...
    for key in keys:
        ...
        if existingUser:
            ...
            db_user = UserDB(
                ...
                address_postal=None,
                # account
                role_id=tpsuperapp.ROLE_METERREADER,
...
async def update_configuration(
    user_id: int, config_update: User, db: AsyncSession = Depends(get_tpsuperapp_db)
):
    result = await db.execute(
        select(UserDB)
        .options(
...
async def get_user_id(user_id: int, db: AsyncSession = Depends(get_tpsuperapp_db)):
...
async def get_user_email(user_email: str, db: AsyncSession = Depends(get_tpsuperapp_db)):
...
async def list_users(db: AsyncSession = Depends(get_tpsuperapp_db)):
    result = await db.execute(
        select(UserDB)
        .options(
            selectinload(UserDB.stage),
...
async def list_meter_reader_users(db: AsyncSession = Depends(get_tpsuperapp_db)):
    result = await db.execute(
        select(UserDB)
        .options(
            selectinload(UserDB.stage),
        ...
        .where(UserDB.role_id == tpsuperapp.ROLE_METERREADER)
        .order_by(UserDB.email)
    )
    return result.scalars().all()
