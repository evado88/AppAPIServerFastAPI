...
async def create_type(route: WalkRoute, db: AsyncSession = Depends(get_tpsuperapp_db)):
    # check user exists
    ...
    try:
        ...
        await db.rollback()
        raise HTTPException(status_code=400, detail=f"Unable to create route: f{e}")
    return db_user
...
async def initialize(db: AsyncSession = Depends(get_tpsuperapp_db)):
    ...
    try:
        ...
        await db.rollback()
        raise HTTPException(
            status_code=400, detail=f"Unable to initialize walk routes: f{e}"
        )
    return {
        "succeeded": True,
...
async def get_knowledgebase_category(
    route_id: int, db: AsyncSession = Depends(get_tpsuperapp_db)
):
    ...
    if not category:
        raise HTTPException(
            status_code=404, detail=f"Unable to find route with id '{route_id}'"
        )
    return category
...
async def update_category(
    route_id: int, route_update: WalkRoute, db: AsyncSession = Depends(get_tpsuperapp_db)
):
    result = await db.execute(
        select(WalkRouteDB)
        .options(
...
async def list_routes(db: AsyncSession = Depends(get_tpsuperapp_db)):
    result = await db.execute(
        select(WalkRouteDB)
        .options(
            selectinload(WalkRouteDB.user),
