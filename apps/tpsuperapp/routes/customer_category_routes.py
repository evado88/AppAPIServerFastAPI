...
from apps.tpsuperapp.models.customer_category_model import Category, CategoryDB, CategoryWithDetail
...
async def post__category(category: Category, db: AsyncSession = Depends(get_tpsuperapp_db)):
    # check user exists
...
async def update_category(
    cat_id: int, category_update: Category, db: AsyncSession = Depends(get_tpsuperapp_db)
):
...
async def list__categories(db: AsyncSession = Depends(get_tpsuperapp_db)):
    result = await db.execute(
        select(CategoryDB)
        .options(
            selectinload(CategoryDB.user),
...
async def get__category(cat_id: int, db: AsyncSession = Depends(get_tpsuperapp_db)):
    result = await db.execute(
        select(CategoryDB)
        .options(
            selectinload(CategoryDB.user),
