...
from apps.tpsuperapp.models.user_role_model import UserRole, UserRoleDB

router = APIRouter(prefix="/user-roles", tags=["Roles"])


@router.post("/create", response_model=UserRole)
async def create_userrole(userrole: UserRole, db: AsyncSession = Depends(get_tpsuperapp_db)):
...
async def list_userrolees(db: AsyncSession = Depends(get_tpsuperapp_db)):
