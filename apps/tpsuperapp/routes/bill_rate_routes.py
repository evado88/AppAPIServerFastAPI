...
from apps.tpsuperapp.models.bill_rate_model import (
    BillRate,
    BillRateDB,
    BillRateWithDetail,
)
...
async def create_type(billrate: BillRate, db: AsyncSession = Depends(get_tpsuperapp_db)):
    # check user exists
...
async def get_knowledgebase_category(
    billrate_id: int, db: AsyncSession = Depends(get_tpsuperapp_db)
):
...
async def update_category(
    billrate_id: int, billrate_update: BillRate, db: AsyncSession = Depends(get_tpsuperapp_db)
):
    result = await db.execute(select(BillRateDB).where(BillRateDB.id == billrate_id))
...
async def list_billrates(db: AsyncSession = Depends(get_tpsuperapp_db)):
    result = await db.execute(select(BillRateDB))
    return result.scalars().all()

@router.get("/items", response_model=List[BillRate])
async def list_billrates_simple(db: AsyncSession = Depends(get_tpsuperapp_db)):
    result = await db.execute(select(BillRateDB))
    return result.scalars().all()