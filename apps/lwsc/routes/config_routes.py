...
from apps.lwsc.models.configuration_model import AppConfiguration, AppConfigurationDB, SACCOConfigurationWithDetail
from apps.lwsc.models.user_model import UserDB
from apps.lwsc.models.status_types_model import StatusTypeDB

router = APIRouter(prefix="/app-config", tags=["AppConfiguration"])


@router.post("/create", response_model=AppConfiguration)
async def post_config(config: AppConfiguration, db: AsyncSession = Depends(get_lwsc_db)):
    # check user exists
...
async def update_configuration(config_id: int, config_update: AppConfiguration, db: AsyncSession = Depends(get_lwsc_db)):
...
async def list_configurations(db: AsyncSession = Depends(get_lwsc_db)):
    result = await db.execute(
