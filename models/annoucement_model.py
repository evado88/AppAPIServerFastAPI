...
class AnnouncementDB(Base):
    ...
    stage = relationship(
        "ReviewStageDB", back_populates="announcements", lazy="selectin"
    )
    status = relationship(
        "StatusTypeDB", back_populates="announcements", lazy="selectin"
    )
    attachment = relationship(
        "AttachmentDB", back_populates="announcements", lazy="selectin"
    )
...
class Announcement(BaseModel):
    ...
    title: str = Field(
        ...
        min_length=2,
        max_length=50,
        description="Title must be between 2 and 50 characters",
    )
    content: str = Field(
        ..., min_length=10, description="The content must be at least 10 characters"
    )
